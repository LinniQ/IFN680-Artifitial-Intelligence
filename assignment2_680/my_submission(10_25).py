...
def train_test_Data(input_dim, file_name, epochs, train_E):
    with np.load(file_name) as npzfile:
        x_train = npzfile['x_train']
        y_train = npzfile['y_train']
        x_test = npzfile['x_test']
...
#------------------------------------------------------
    if(train_E):
        ...
        np.savez('warped_data1.npz', x_train = x_train, y_train = y_train, x_test = x_test, y_test = y_test)
        file_name = 'warped_data1.npz'
        train_test_Data(input_dim, file_name, epochs, train_E = False)
    model.fit([tr_pairs[:, 0], tr_pairs[:, 1]], tr_y,
          batch_size=128,
          epochs=epochs,
          validation_data=([te_pairs[:, 0], te_pairs[:, 1]], te_y))
    np.savez('final_data1.npz', x_train = x_train, y_train = y_train, x_test = x_test, y_test = y_test)
    file_name = 'final_data1.npz'
...
if __name__=='__main__':
    '''
    input_dim,file_name = initial();
    train_test_Data(input_dim,file_name,1,False);
    input_dim,file_name = initial();
    file_name = warped_data(45,0.3,input_dim,100000,file_name);
    train_test_Data(input_dim,file_name,1,False);
    '''
    ...
    train_test_Data(input_dim,file_name,1,True);
