...
class PatternPosePopulation(population_search.Population):
    ...
    def evaluate(self):
        ...
        search = np.zeros(self.n)
        for i in range(self.n):
            tup = self.pat.evaluate(self.distance_image,self.W[i])
...
def test_particle_filter_search():
    ...
    Lw, Lc = pop.particle_filter_search(150,log=True)  #Lw each individual(x,y,theta,scale)
